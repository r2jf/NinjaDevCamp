$(function() {
        ...
        query.read().then(function(todoItems) {
            var listItems = $.map(todoItems, function(item) {
                return $('<li>')
                    ...
                    .append($('<div>').append($('<input class="item-text">').val(item.ciudad + ', ' + item.ff)));
            });
    ...
    $('#add-item').submit(function(evt) {
        ...
        if (itemText !== '') {
            todoItemTable.insert({ ciudad: itemText, mostrar: true }).then(refreshTodoItems, handleError);
        }
        textbox.val('').focus();
        evt.preventDefault();
    });
    ...
    $(document.body).on('change', '.item-text', function() {
        ...
        todoItemTable.update({ id: getTodoItemId(this), ciudad: newText }).then(null, handleError);
    });
